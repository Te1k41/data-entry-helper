...
const LiveCheckRelay = {
    ...
    setRelayState(connected) {
        ...
        if (connected) {
            ...
            this._fillData = null;
            LiveCheck.compareAll(); // retains relay-independent duplicate-IMO checks
        }
    },
    ...
    handleInput(event) {
        if (LiveCheck._ownWriteInProgress) return; // our own setFieldValue() writes also fire "input" -- ignore those, but not other features'
        ...
        if (this.RELEVANT_PATTERN.test(name)) {
            ...
            this._inputDebounceTimer = setTimeout(() => LiveCheck.compareAll(), this.INPUT_DEBOUNCE_MS);
        }
    },
...
LiveCheck._extraComparisons.push((add, fills) => LiveCheckRelay.collectMismatches(add, fills));
Toolbar.register({
    ...
    group: "proof",
    requiresRelay: true,
    onClick: () => LiveCheckRelay.fetchFillData(),
});
onRelayConnectionStatusChange(state => LiveCheckRelay.setRelayState(state === "connected"));
document.addEventListener("input", event => LiveCheckRelay.handleInput(event));
document.addEventListener("change", event => {
    if (LiveCheck._ownWriteInProgress) return;
    const name = event.target.name;
    if (name === "service") {
        LiveCheckRelay.fetchFillData();
    } else if (LiveCheckRelay.RELEVANT_PATTERN.test(name) && !LiveCheck.RELEVANT_PATTERN.test(name)) {
        // Local IMO/One-off changes already reach compareAll through main.js.
        // Keep committed/programmatic name, voyage, date and SP001 changes immediate.
        LiveCheck.compareAll();
    }
}, true);
